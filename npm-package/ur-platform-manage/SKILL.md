---
name: ur-platform-manage
description: "ur-platform-manage — 联犀 SaaS 平台 平台管理 CLI 工具"
metadata:
  hermes:
    tags: [platform, admin, tenant, user, auth, system]
---

# ur-platform-manage — 平台管理

> **配置检查**：如果尚未配置联犀连接，请先运行 `ur-platform-manage login --no-wait`，按指引在浏览器中完成授权。`setup` 命令是终端交互式的，在 AI 聊天环境中无法使用。
...
- **企业管理**: 租户（企业）的全生命周期管理
  API: `/api/v1/system/tenant/info/create`, `/api/v1/system/tenant/info/update`, `/api/v1/system/tenant/info/delete`, `/api/v1/system/tenant/info/get-one`, `/api/v1/system/tenant/info/get-list`
  - **企业列表**: 查看和管理所有租户
  - **企业详情**: 查看租户详情、绑定应用、配置
- **用户管理**: 平台级用户管理（跨租户）
  API: `/api/v1/system/user/info/get-list`, `/api/v1/system/user/info/get-one`, `/api/v1/system/user/info/create`, `/api/v1/system/user/info/update`, `/api/v1/system/user/info/delete`
  - **用户列表**: 查看所有租户下的用户
  - **用户详情**: 查看用户详情和租户绑定
- **应用管理**: 系统应用的创建和配置
  API: `/api/v1/system/app/info/create`, `/api/v1/system/app/info/update`, `/api/v1/system/app/info/get-one`, `/api/v1/system/app/info/get-list`, `/api/v1/system/app/menu/create`, `/api/v1/system/app/menu/get-list`
  - **应用列表**: 查看和管理系统应用
  - **应用菜单**: 管理应用的菜单树
- **授权管理**: 授权码、套餐和续期管理
  API: `/api/v1/system/license/package/get-list`, `/api/v1/system/license/info/get-list`, `/api/v1/system/license/record/get-list`
  - **套餐管理**: 管理授权套餐
  - **授权码管理**: 生成和管理授权码
  - **审计记录**: 查看授权操作记录
- **协议管理**: 用户协议、隐私政策等协议库管理
  API: `/api/v1/system/agreement/create`, `/api/v1/system/agreement/update`, `/api/v1/system/agreement/get-list`, `/api/v1/system/agreement/get-one`
- **系统管理**: 系统设置、权限和日志
  - **系统设置**: 全局系统配置
  - **接口管理**: API 接口注册和管理
  - **资源总览**: 权限资源树管理
  ...
  - **登录日志**: 查看登录日志
  - **操作日志**: 查看操作日志
- **系统开发**: Hook 扩展、字典、数据源、任务管理
  - **Hook 扩展管理**: 管理 Hook 扩展点
  - **字典管理**: 数据字典 CRUD
  - **数据源管理**: 外部数据源配置
  - **查询配置管理**: 数据查询配置
  - **任务管理**: 定时任务管理
- **AI 中台**: AI 模型、智能体、知识库、技能包管理
  - **模型配置**: LLM/TTS/ASR 模型配置
  - **智能体分组**: Agent 分组管理
  - **智能体管理**: Agent CRUD
  - **MCP 服务**: MCP 服务管理
  - **知识库**: 知识库和文档管理
...
共 492 个可调用端点（按 platform/admin/all/public 权限过滤）。
...
```bash
# 配置
ur-platform-manage setup

# 验证连通性
ur-platform-manage check

# 调用 API
ur-platform-manage api /api/v1/system/user/self/get-one
```
