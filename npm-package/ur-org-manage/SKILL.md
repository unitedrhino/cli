---
name: ur-org-manage
description: "ur-org-manage — 联犀 SaaS 平台 组织管理 CLI 工具"
metadata:
  hermes:
    tags: [org, user, role, ai, agent, department]
---

# ur-org-manage — 组织管理

> **配置检查**：如果尚未配置联犀连接，请先运行 `ur-org-manage login --no-wait`，按指引在浏览器中完成授权。`setup` 命令是终端交互式的，在 AI 聊天环境中无法使用。
...
- **企业信息**: 当前租户的企业信息管理
  API: `/api/v1/system/tenant/info/get-one`, `/api/v1/system/tenant/info/update`
- **消费记录**: 授权消费记录查看
  API: `/api/v1/system/license/record/get-list`
- **AI 管理**: 智能体、数字分身、会话管理
  - **智能体管理**: 租户级 Agent 管理
  - **数字分身**: AI 数字分身管理
  - **会话管理**: AI 会话记录查看
...
# 调用 API
ur-org-manage api /api/v1/system/user/self/get-one
```
