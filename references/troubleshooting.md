...
metadata:
  hermes:
    tags: [troubleshooting, error, debug, support]
---
...
| 场景 | 行为 |
|------|------|
| 用户属于企业，企业已绑定 App | ✅ 登录成功 |
| 用户属于企业，企业**未**绑定 App | ❌ "该企业未绑定该应用，无法登录" |
| 用户不属于企业，App 是 `client` 类型 | 自动注册到 App 所属企业 |
| 用户不属于企业，App 是 `admin` 类型 | ❌ 报错 |
| 不带企业登录，用户有绑定该 App 的企业 | ✅ 登录成功 |
| 不带企业登录，用户无绑定该 App 的企业（admin 应用） | ❌ "您没有权限访问该应用" |
